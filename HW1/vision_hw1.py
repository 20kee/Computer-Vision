...
im5_array = im2_array.copy()
for x in range(0,150):
    for y in range(0,150):
        im5_array[y,x] = im5_array[y,x] * (grad[y, x]/155)
        
im6 = Image.fromarray(im5_array.astype('uint8'))
im6.save('gradation.png','PNG')
# convert to uint8 and then to a PIL image and save
im5 = Image.fromarray(grad.astype('uint8'))
print(im5.width, im5.height)
